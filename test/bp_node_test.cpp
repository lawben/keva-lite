...
TEST_F(BPNodeTest, SplitInternalNodeFront) {
  ...
  const std::vector<NodeID> expected_old_values = {0, 12, 24};
  EXPECT_EQ(node.children(), expected_old_values);
}
...
TEST_F(BPNodeTest, SplitInternalNodeMiddleStays) {
  ...
  const std::vector<NodeID> expected_old_values = {12, 18, 24};
  EXPECT_EQ(node.children(), expected_old_values);
}
...
TEST_F(BPNodeTest, SplitInternalNodeOddNumberKeys) {
